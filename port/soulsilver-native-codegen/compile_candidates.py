...
cc=Path(os.environ.get('PSPDEV',Path.home()/'pspdev'))/'bin/psp-gcc'
def compile_one(r):
 ...
 p=subprocess.run([str(cc),'-O2','-G0','-std=gnu99','-ffunction-sections','-fdata-sections','-c',str(src),'-o',str(obj)],capture_output=True,text=True)
