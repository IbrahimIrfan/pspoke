...
BOOL SND_ReadInstData (const SNDBankData * bank, int prgNo, int key, SNDInstData * inst)
{
    ...
    {
        #ifdef SDK_PORT
        const SNDInstParam * param = (const SNDInstParam *)((u8 *)bank + instOffset + sizeof(SNDBankData) - sizeof(WIN_SNDBankData));
        #else
    ...
    {
        #ifdef SDK_PORT
        const SNDDrumSet * drumSet = (const SNDDrumSet *)((u8 *)bank + instOffset + sizeof(SNDBankData) - sizeof(WIN_SNDBankData));
        #else
    ...
    case SND_INST_KEY_SPLIT:
    {
        int index = 0;
        #ifdef SDK_PORT
        const SNDKeySplit * keySplit = (const SNDKeySplit *)((u8 *)bank + instOffset + sizeof(SNDBankData) - sizeof(WIN_SNDBankData));
        #else
