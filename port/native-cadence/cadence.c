...
void PSPNativeVBlankWait(void){uint64_t now=sceKernelGetSystemTimeWide();if(!ready){epoch=now;sequence=1;ready=1;}uint64_t target=deadline();if(now<target)idleUs+=target-now;while(now<target){uint64_t diff=target-now;unsigned part=diff>1000000?1000000:(unsigned)diff;sceKernelDelayThread(part);now=sceKernelGetSystemTimeWide();}sequence++;}
