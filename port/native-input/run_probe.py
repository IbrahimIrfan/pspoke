...
stage=base/'memstick/PSP/GAME/NativeInput';stage.mkdir(parents=True,exist_ok=True)
...
text=(base/'run.log').read_text();assert '[INPUT-RTC] failures=0' in text;assert '[STARTUP] validation failures=0 VBlankTasks=1 frame=1 FS=1' in text
print('\n'.join(line for line in text.splitlines() if '[STARTUP]' in line or '[INPUT-RTC]' in line))
