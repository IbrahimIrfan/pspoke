...
static inline void MI_CpuFillFast (void * dest, u32 data, u32 size)
{
  ...
  #ifdef SDK_PORT
  /* Match the DS MIi_CpuClearFast: it stores whole words while the pointer is below dest+size, so a
     size that is not a multiple of 4 still writes the last (partial) word. size/4 dropped it, which
     left the 1-byte healthbox->state clears undone (new Pokemon had no HP bar after switching). */
  u32 *words = (u32 *)dest;
  for (u32 i = 0; i < (size + 3) / 4; i++) words[i] = data;
  #else
