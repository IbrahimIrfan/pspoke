...
stage=base/'memstick/PSP/GAME/NativeFonts';stage.mkdir(parents=True,exist_ok=True)
...
with (base/'run.log').open('w') as log:
 subprocess.run([ppsspp,'--memstick='+str(base/'memstick'),'--timeout=5','--log','--loglevel=4','--graphics=software','--screenshot-save='+str(base/'fonts.png'),str(stage/'EBOOT.PBP')],cwd=base,stdout=log,stderr=subprocess.STDOUT,check=True)
text=(base/'run.log').read_text();assert '[FONT] lazy/immediate glyph cases=42 failures=0' in text;assert '[STARTUP] validation failures=0 VBlankTasks=1 frame=1 FS=1' in text
print('\n'.join(line for line in text.splitlines() if '[STARTUP]' in line or '[FONT]' in line))
