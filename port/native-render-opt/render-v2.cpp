...
static int Present(bool waitForVblank){
 ...
 mapMemory();if(drawB&&(power&0x8000)&&BottomSame(power))drawB=false;engineA.Enabled=(power&2)!=0;engineB.Enabled=(power&512)!=0;bindRegisters(engineA,0);bindRegisters(engineB,0x1000);
 ...
 for(unsigned line=0;line<192;line++){GPU::VCount=line;engineA.CheckWindows(line);renderer.DrawSprites(line,&engineA);renderer.DrawScanline(line,&engineA);
  if(drawB){engineB.CheckWindows(line);renderer.DrawSprites(line,&engineB);renderer.DrawScanline(line,&engineB);}}
 if(drawB)SaveBottom(power);
