...
int main(){scePowerSetClockFrequency(333,333,166);scene();s_reg_GX_POWCNT=0x820f;
 ...
 PSPNativeRenderInit();unsigned failures=0,skipped=0,active=0;for(unsigned f=0;f<96;f++){
  ...
  if(f==65)s_reg_GX_DISPCNT&=~(1<<3);
  if(f==67)((u16*)s_HW_BG_PLTT)[200]^=1;
  if(f==68)s_HW_BG_VRAM[0x70000]^=1;
  if(f==69)((u16*)s_HW_OAM)[127*4+1]^=1;
  if(f==70)s_reg_G2_BG1HOFS=1;
  if(f==71){s_reg_GX_DISPCNT|=1<<3;s_reg_G2_BG1HOFS=0;}
  ...
  if(f>=65&&f<=71){printf("[TOPCACHE] frame=%u mask=%u bind=%u draw=%u\n",f,mask,RenderStage(0),RenderStage(1));if((f==66?0:1)!=(mask&1))failures++;}
