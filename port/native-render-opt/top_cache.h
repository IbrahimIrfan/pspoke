...
static bool TopSame(unsigned power,unsigned disp){
 ...
 if(!SameWords(topCache.regs,PSPNative_GfxRegisters,0x70)||!SameWords(topCache.palette,GPU::Palette,512))return false;
...
static void SaveTop(unsigned power,unsigned disp){
 ...
 memcpy(topCache.regs,PSPNative_GfxRegisters,0x70);memcpy(topCache.palette,GPU::Palette,512);memcpy(topCache.bg,s_HW_BG_VRAM,0x80000);
 bool objects=(disp&((1<<12)|(1<<15)))!=0;
