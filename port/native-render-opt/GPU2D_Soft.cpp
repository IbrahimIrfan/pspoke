...
void SoftRenderer::DrawScanline(u32 line, Unit* unit)
{
...
#if defined(MELONDS_PSP) && defined(PSP_2D_HALF_HEIGHT) && PSP_2D_HALF_HEIGHT
    // Optional 256x96 presentation. Display capture must retain the exact DS
    // scanlines because games can read its result back as emulated VRAM.
    // Mode 2 reduces engine A only, leaving engine B text at full height.
    if ((n3dline & 1) && !GPU3D::CurrentRenderer->Accelerated &&
        !CurUnit->CaptureLatch && !(CurUnit->CaptureCnt & (1u << 31)) &&
        (PSP_2D_HALF_HEIGHT != 2 || CurUnit->Num == 0) &&
        ((CurUnit->DispCnt >> 16) & (CurUnit->Num ? 1 : 3)) == 1)
    {
        // Affine reference advancement normally happens inside the BG draw
        // routines. Preserve it even though this output scanline is repeated.
        if (!(CurUnit->DispCnt & (1 << 7)))
        {
            const u32 mode = CurUnit->DispCnt & 7;
            if ((CurUnit->DispCnt & 0x0800) && mode >= 1 && mode <= 5)
            {
                CurUnit->BGXRefInternal[1] += CurUnit->BGRotB[1];
                CurUnit->BGYRefInternal[1] += CurUnit->BGRotD[1];
            }
            if ((CurUnit->DispCnt & 0x0400) &&
            ...
            else CurUnit->BGMosaicY++;
        }
        CurUnit->UpdateMosaicCounters(line);
        memcpy(dst, dst - stride, 256 * sizeof(u32));
        return;
    }
#endif
    ...
    case 1: // regular display
        {
#ifdef MELONDS_PSP
            // With no master brightness, convert straight from the composed line.
            // Display capture reads BGOBJLine independently below.
            if (!GPU3D::CurrentRenderer->Accelerated && !(CurUnit->MasterBrightness & 0xC000))
            {
                outputSrc = BGOBJLine;
                break;
            }
...
void SoftRenderer::DrawScanline_BGOBJ(u32 line)
{
    ...
    if (!GPU3D::CurrentRenderer->Accelerated)
    {
        // Without a second target, forced sprite/3D blending is impossible.
        // Without a first target or effect mode, ordinary effects are too.
...
void SoftRenderer::DrawBG_Text(u32 line, u32 bgnum)
{
    ...
    if (bgcnt & 0x0080)
    {
        ...
        if (!mosaic)
        {
            const u32 flag = 0x01000000u << bgnum;
            const u32 windowBit = 1u << bgnum;
            const bool windows = (CurUnit->DispCnt & 0xE000) != 0;
            int i = 0;
            while (i < 256)
            {
                u32 inTile = xoff & 7;
                u32 count = 8 - inTile;
                if (count > (u32)(256-i)) count = 256-i;
                ...
                pixelsaddr = tilesetaddr + ((curtile & 0x03FF) << 6)
                             + (((curtile & 0x0800) ? (7-(yoff&7)) : (yoff&7)) << 3);
                // Eight-byte rows cannot cross the wrapped VRAM boundary.
                const u8* pixel = &bgvram[pixelsaddr & bgvrammask];
                const int step = (curtile & 0x0400) ? -1 : 1;
                int pos = (curtile & 0x0400) ? (7-inTile) : inTile;
                const int end = i + count;
                xoff += count;
                if (windows)
                {
                    for (; i < end; i++, pos += step)
                    {
                        u32 index = pixel[pos];
                        if (index && (WindowMask[i] & windowBit))
                            drawPixel(&BGOBJLine[i], curpal[index], flag);
                    }
                }
                else
                {
                    ...
                    {
                        u32 index = pixel[pos];
                        if (index) drawPixel(&BGOBJLine[i], curpal[index], flag);
                    }
                }
            }
            return;
...
#ifdef MELONDS_PSP
        // A text row is one aligned 32-bit word. Decode a tile span at a
        // time so tile selection, horizontal flip and address wrapping do
        // not need to be repeated for each pixel. Mosaic keeps the scalar path.
        if (!mosaic)
        {
            const u32 flag = 0x01000000u << bgnum;
            const u32 windowBit = 1u << bgnum;
            ...
            while (i < 256)
            {
                ...
                pixelsaddr = tilesetaddr + ((curtile & 0x03FF) << 5)
                             + (((curtile & 0x0800) ? (7-(yoff&7)) : (yoff&7)) << 2);
                u32 row;
                memcpy(&row, &bgvram[pixelsaddr & bgvrammask], sizeof(row));
                if (curtile & 0x0400)
                {
                    row = __builtin_bswap32(row);
                    row = ((row & 0xF0F0F0F0u) >> 4) | ((row & 0x0F0F0F0Fu) << 4);
                }
                row >>= inTile * 4;
                const int end = i + count;
                xoff += count;
