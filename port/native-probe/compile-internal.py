...
def comp(p):
 ...
 if p.suffix=='.cpp':f[0]=f[0].replace('psp-gcc','psp-g++');f=[x for x in f if x!='-std=gnu99'];f+=['-std=gnu++03','-fpermissive'];idx=f.index('-include');del f[idx:idx+2]
...
subprocess.run(['@PSPDEV@/bin/psp-ar','rcs',str(a)]+[str(p) for p in out.glob('*.o')],check=True)
