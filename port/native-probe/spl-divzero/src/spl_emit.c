...
void SPLEmitter_EmitParticles(SPLEmitter *emtr, SPLParticleList *list)
{
    ...
    for (i = 0; i < totalEmissions; i++) {
        ...
        if (header->flags.hasTexAnim && res->texAnim->param.randomizeInit) {
            ptcl->texture = res->texAnim->textures[res->texAnim->param.frameCount ? SPLRandom_U32(12) % res->texAnim->param.frameCount : 0];
        ...
        ptcl->loopTimeFactor = res->header->misc.loopFrames ? 0xFFFF / res->header->misc.loopFrames : 0;
...
void SPLEmitter_EmitChildren(SPLParticle *ptcl, SPLEmitter *emtr, SPLParticleList *list)
{
    ...
    for (int i = 0; i < childRes->misc.emissionCount; i++) {
        ...
        child->loopTimeFactor = (ptcl->lifeTime / 2) ? 0xFFFF / (ptcl->lifeTime / 2) : 0;
        child->lifeTimeFactor = ptcl->lifeTime ? 0xFFFF / ptcl->lifeTime : 0;
