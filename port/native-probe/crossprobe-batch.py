...
subprocess.run(['@PSPDEV@/bin/psp-ar','rcs',str(b/'libplatinum-probe.a')]+objs,check=True)
with (b/'object-sizes.txt').open('w') as f:subprocess.run(['@PSPDEV@/bin/psp-size']+objs,stdout=f,check=True)
