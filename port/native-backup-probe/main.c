...
if(!PSPNative_OpenBackup("probe-save.bin")){printf("[BACKUP] open failed\n");return 1;}
...
CARD_UnlockBackup(12);if(!PSPNative_CloseBackup()||!PSPNative_OpenBackup("probe-save.bin"))failures++;
CARD_LockBackup(12);if(!CARD_ReadFlash(255,output,777)||memcmp(input,output,777))failures++;CARD_UnlockBackup(12);if(!PSPNative_CloseBackup())failures++;
CARD_ReadFlashAsync(0,output,1,Done,&one);if(callbacks!=4||callbackResult!=CARD_RESULT_NO_RESPONSE)failures++;
if(PSPNative_OpenBackup("does-not-exist.bin"))failures++;
