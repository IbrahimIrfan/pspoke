...
static bool rawMode=false;static unsigned rawMatrixGen;
/* Display-list geometry cache support (frontend.cpp). While a list is being recorded the
   vertices it appends are the last g3RecCount entries of the chunk; any flush in between
   (matrix change, CPU-path polygon, texture change) invalidates the recording. */
...
/* Direct draw from cache memory (16-byte aligned, written back once when stored): pending
   chunk vertices are flushed first so GE order is unchanged; the same texture/cull state
   applies as would have at the later flush (state changes always flush before applying). */
...
#endif
static bool cullEnabled=false,cullAll=false;
extern "C" {
...
static void APIENTRY GuCull(GLenum c){cullAll=c==GL_FRONT_AND_BACK;sceGuFrontFace(c==GL_FRONT?GU_CCW:GU_CW);}
 ...
 *g3ChunkPtr++={v->s*s_texImageParam.textureSSize,v->t*s_texImageParam.textureTSize,channel(v->r)|(channel(v->g)<<8)|(channel(v->b)<<16)|(channel(v->a)<<24),
...
extern "C" unsigned g3DrawUs=0,g3DrawN=0;unsigned g3HwDrawAll=0;
static void DrawVertices(const GuVertex*v,unsigned count,bool copy){
 ...
 bool drawS=((++g3HwDrawAll)&7)==0;unsigned t0=drawS?sceKernelGetSystemTimeLow():0;
#endif
 ...
 /* The 3D buffer's alpha channel is the PSP stencil. Opaque batches stamp 255 (unchanged).
    Translucent batches (polygon alpha < 31, or texels with partial alpha) must leave DS-style
    coverage there instead, otherwise the 2D compositor pastes them as opaque colour that was
    blended against the black clear (dark rays/flashes in battle effects). Colour pass keeps
    the stencil; then 8 stencil-only passes quantise fragment alpha and keep the maximum
    (stencil GREATER + REPLACE), matching the DS rule alpha = max(src, dst). The colour
    already holds src*a (+ dst*(1-a)), i.e. premultiplied; Draw3DLayer composites with ONE. */
...
#elif defined(PSP_NATIVE_G3_HWPROF)
 if(drawS){g3DrawUs+=sceKernelGetSystemTimeLow()-t0;g3DrawN++;}
#endif
}
extern "C" void G3SIM_DrawArray() {DrawVertices(vertices,(unsigned)(g3ChunkPtr-vertices),true);}
extern "C" void G3SIM_DrawCleanUp() {ResetVertices();rawMatrixGen=0;}
...
extern "C" void PSPNativeRawMatrix(const s32*m,unsigned gen){
 ...
 sceGuSetMatrix(GU_PROJECTION,&clip);sceGuSetMatrix(GU_VIEW,&identity);sceGuSetMatrix(GU_MODEL,&identity);
}
