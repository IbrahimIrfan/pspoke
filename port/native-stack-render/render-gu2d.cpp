...
#include <malloc.h>
#include <array>
extern "C" void PSPNativeFatal(const char*);
...
extern "C" void* PSPNativeGUGetMemory(unsigned bytes){
 unsigned used=sceGuCheckList();if(used+bytes+4096>sizeof(list)){char b[96];snprintf(b,sizeof(b),"GU display list exhausted used=%u request=%u capacity=%u",used,bytes,(unsigned)sizeof(list));PSPNativeFatal(b);}
...
// GE wait profile: time the CPU spends in sceGuSync (hardware-only cost, ~0 under PPSSPP) and mid-frame texture fences.
...
// On-screen placement of the two DS panels (pixels on the 480x272 display).
// Main panel fills the full height (363x272, ~1.42x nearest-neighbour, no black bars); the touch panel
// (117x88) is centred in the remaining width. The bottom-right corner is left free for the DEV fps counter.
...
extern "C" void PSPNativeGUTextureFence(){
 ...
 if(!openFrame){PSPNativeFatal("GE texture fence outside frame");}
...
static unsigned atlasBytes,atlasCount,atlasHighBytes,atlasAllocCalls,atlasAllocFails;
// Instrumentation + survivable fallback. A missing atlas must never be written
// through: callers get a shared scratch page instead, and the frame renders
// wrong rather than corrupting memory or killing the process.
static u8 atlasFallback[65536] __attribute__((aligned(64)));
extern "C" void PSPNativeRenderMemStats(unsigned*count,unsigned*bytes,unsigned*high,
                                        unsigned*calls,unsigned*fails,unsigned*guHigh,unsigned*guCap){
 if(count)*count=atlasCount;if(bytes)*bytes=atlasBytes;if(high)*high=atlasHighBytes;
 if(calls)*calls=atlasAllocCalls;if(fails)*fails=atlasAllocFails;
 if(guHigh)*guHigh=highWater;if(guCap)*guCap=(unsigned)sizeof(list);
}
static inline bool AtlasIsFallback(const u8*d){return d==atlasFallback;}
static u8*AtlasData(Atlas&a,unsigned bytes=65536){
 if(a.data){if(a.capacity!=bytes){PSPNativeFatal("atlas capacity mismatch");}return a.data;}
 atlasAllocCalls++;
 a.data=(u8*)memalign(64,bytes);
 if(!a.data){PSPNativeG3TrimCache();a.data=(u8*)memalign(64,bytes);}
 if(!a.data){
  atlasAllocFails++;
  // Loud, but rate limited: this can otherwise fire for every atlas every frame
  // and the printf storm would itself look like a hang.
  if(atlasAllocFails<=8||atlasAllocFails%600==0)
   printf("[ATLAS] allocation FAILED bytes=%u live=%u count=%u maxfree=%u totalfree=%u fails=%u\n",
          bytes,atlasBytes,atlasCount,(unsigned)sceKernelMaxFreeMemSize(),
          (unsigned)sceKernelTotalFreeMemSize(),atlasAllocFails);
  // Do not abort and do not return NULL: hand back a zeroed scratch page that is
  // always at least as large as any atlas we allocate (64 KiB).
  memset(atlasFallback,0,bytes<sizeof(atlasFallback)?bytes:sizeof(atlasFallback));
  return atlasFallback;
 }
 a.capacity=bytes;atlasBytes+=bytes;atlasCount++;
 if(atlasBytes>atlasHighBytes)atlasHighBytes=atlasBytes;
 memset(a.data,0,bytes);return a.data;
}
...
static u8*BuildBGAtlas(int engine,int slot,const u8*vram,u32 mask,u32 tilesetaddr,bool bpp8){
 ...
 auto matches=[&](const Atlas&v){return v.data&&v.gen==frameGen&&v.source==vram&&v.tilebase==tilesetaddr&&v.sourceMask==mask&&v.bpp8==bpp8;};
 if(matches(a))return a.data;
 for(int s=0;s<slot;s++)if(matches(slots[s]))return slots[s].data;
 u8*d=AtlasData(a);
 // Out of memory: draw from a blank page this frame and retry the allocation next frame.
 if(AtlasIsFallback(d)){a.gen=0;return d;}
 a.gen=frameGen;a.source=vram;a.tilebase=tilesetaddr;a.sourceMask=mask;a.bpp8=bpp8;
...
static u8*ObjTile(int e,u32 byteaddr,bool bpp8,const u8*vram,u32 mask,u32*tileOut){
 if(bpp8){Atlas&a=objAtlas8[e][(byteaddr>>16)&3];u8*d=AtlasData(a);u32 n=(byteaddr>>6)&1023;*tileOut=n;
  if(AtlasIsFallback(d))return d;   // no atlas this frame: blank tile, stamp left stale so it converts later
...
static bool EngineSupported(const GPU2D::Unit&u){
 ...
 if(d&0x6000){                                // WIN0/WIN1: only non-wrapping rectangles on the GE path
...
static void DrawAffineBG(GPU2D::Unit&u,int bg,bool blend){
 ...
 if(!identity){
  ...
  // atlasFallback is 65536 bytes, larger than the 256*192 this path writes.
  for(int y=0;y<192;y++){
...
static void Draw3DLayer(bool blend){
 ...
 /* 3D colour is premultiplied by coverage (see g3_backend.cpp DrawVertices): src*1 + dst*(1-a). */
...
static void RenderEngineSoft(GPU2D::Unit&u,bool need3D){
 ...
 for(unsigned line=0;line<192;line++){GPU::VCount=line;u.CheckWindows(line);renderer.DrawSprites(line,&u);renderer.DrawScanline(line,&u);}
...
// Defined by the application (osk.c); weak so this library still links into
// builds that do not carry the keyboard bypass.
...
/* ---- OPT_GE_ASYNC: pipelined 2D/display pass --------------------------------------------
   ...
   atlas fallback page, CLUT arrays, raw[] soft engines) is written only inside Present after
...
static unsigned AsyncResourceHash(unsigned used){
 ...
 h=HashBytes(h,atlasFallback,sizeof(atlasFallback));
...
static int Present(bool waitForVblank){
 ...
 lastDraw=3;
 // present both panels
 const bool oskOwnsScreen = PSPNativeOskIsActive && PSPNativeOskIsActive();
 /* The keyboard flickers because the game and the dialog share one buffer while
    it is up. Rendering the game into the hidden buffer instead cured the flicker
    but hid the dialog too - the firmware draws into whatever sceGuDrawBufferList
    last selected, not into the published buffer - so keep the flicker for now. */
 sceGuDrawBufferList(GU_PSM_8888,(void*)(uintptr_t)displayOffset,512);
 ...
 sceGuClearColor(0xff101010);sceGuClear(GU_COLOR_BUFFER_BIT);sceGuEnable(GU_TEXTURE_2D);sceGuTexMode(GU_PSM_8888,0,0,0);sceGuTexFunc(GU_TFX_REPLACE,GU_TCC_RGBA);sceGuTexWrap(GU_CLAMP,GU_CLAMP);
 /* Layout (user request): big crisp main screen on the left, small touch screen on
    the right. Main is upscaled with nearest (user prefers crisp pixels); the touch
    panel is downscaled to about half, where nearest would drop pixels, so linear. */
 /* In stylus mode the touch screen is what the player is working on, so the two
    panels swap places: touch screen gets the big left slot, main screen the small one. */
 ...
 convertUs=sceKernelGetSystemTimeLow()-t2;last2DUs=sceKernelGetSystemTimeLow()-t0;lastReadbackUs=0;
 // PSP on-screen keyboard (name entry bypass). The firmware dialog has to be
 // ticked once per displayed frame, after this frame's GE work is flushed and
 // before the buffer is handed to the display, so this is the only correct
 // place for it. While it is up we stop flipping buffers: the dialog draws
 // into whatever sceDisplaySetFrameBuf last published, and the game behind it
 // is frozen anyway, so a single buffer keeps the two in step.
 bool oskActive=oskOwnsScreen;
