...
#define G3OP_SAMPLE() ((void)0)
#define G3OP_RUN(call) call
#define G3CAT_END(c) (g3CatUs[c]+=sceKernelGetSystemTimeLow()-catT0,g3CatN[c]++)
#elif defined(PSP_NATIVE_G3_HWPROF)
/* Hardware profile (card log): time one display list in eight by category, and every op of a
   sampled plain/record list. Two timer syscalls per sampled list only. */
extern "C" void PSPNativeMemLog(const char*,...);
static unsigned hwCatUs[3],hwCatN[3],hwCatAll[3],hwListN,hwFrames,hwImmUs,hwOpUs[64],hwOpN[64];
extern "C" unsigned g3DrawUs,g3DrawN;extern unsigned g3HwDrawAll;
#define G3CAT_BEGIN() bool catS=((++hwListN)&7)==0;unsigned catT0=catS?sceKernelGetSystemTimeLow():0
/* Per-op timing: one list in 256 (x256 scaling), on every decode path (fast-decoded ops never reached the old
   generic-path timer, so the op line was empty). Type captured before the handler may rewrite it. */
#define G3OP_SAMPLE() bool opS=(hwListN&255)==0
#define G3OP_RUN(call) do{if(opS){unsigned ty_=tempMsg.type&63,ot_=sceKernelGetSystemTimeLow();call;hwOpUs[ty_]+=sceKernelGetSystemTimeLow()-ot_;hwOpN[ty_]++;}else{call;}}while(0)
#define G3CAT_END(c) (hwCatAll[c]++,catS?(hwCatUs[c]+=sceKernelGetSystemTimeLow()-catT0,hwCatN[c]++):0u)
#define G3STAT_LIST(t) ((void)0)
#define G3STAT_IMM(t) ((void)0)
static inline void G3StatsList(const void*,unsigned){}
#ifdef OPT_TEX_DIRTY
extern "C" void PSPNativeTexDirtyLine(unsigned);
#endif
static unsigned HwEst(unsigned c){return hwCatN[c]?(unsigned)((unsigned long long)hwCatUs[c]*hwCatAll[c]/hwCatN[c]):0;}
static void G3StatsSwap(){
 if(++hwFrames%600)return;
 PSPNativeMemLog("[G3HW] us/frame hit=%u(%u) record=%u(%u) plain=%u(%u) imm=%u draw=%u(%u) content_hits=%u",
  HwEst(0)/600,hwCatAll[0]/600,HwEst(1)/600,hwCatAll[1]/600,HwEst(2)/600,hwCatAll[2]/600,hwImmUs/600,
  g3DrawN?(unsigned)((unsigned long long)g3DrawUs*g3HwDrawAll/g3DrawN/600):0,g3HwDrawAll/600,contentHits/600);
 static const char*const nm[48]={"nop","D0","D1","D2","D3","MODE","PUSH","POP","STORE","RESTORE","IDENT","SCALE","TRANS","LOAD44","LOAD43","MUL33","MUL43","MUL44","COLOR","N","T","VTX","V10","V16","VXY","VXZ","VYZ","VDIFF","PATTR","TEXP","TPLT","MDA","MSE","LVEC","LCOL","BEGIN","END","SWAP","VIEW","BOX","POS","VEC","DIFAMB","SPEEMI","SHIN","DUMMY","LIST","?"};
 char line[640];int n=snprintf(line,sizeof line,"[G3HW] sampled-list op us/frame(x256):");
 for(int i=0;i<64;i++){unsigned us=(unsigned)((unsigned long long)hwOpUs[i]*256/600),cnt=(unsigned)((unsigned long long)hwOpN[i]*256/600);if(us>=5&&n<(int)sizeof line-32)n+=snprintf(line+n,sizeof line-n," %s=%u(%u)",nm[i<48?i:47],us,cnt);}
 PSPNativeMemLog("%s",line);
#ifdef OPT_TEX_DIRTY
 PSPNativeTexDirtyLine(600);
#endif
 memset(hwCatUs,0,sizeof hwCatUs);memset(hwCatN,0,sizeof hwCatN);memset(hwCatAll,0,sizeof hwCatAll);memset(hwOpUs,0,sizeof hwOpUs);memset(hwOpN,0,sizeof hwOpN);
 hwImmUs=0;g3DrawUs=g3DrawN=0;g3HwDrawAll=0;contentHits=contentStores=0;
}
#else
#define G3OP_SAMPLE() ((void)0)
#define G3OP_RUN(call) call
...
#endif
#ifdef PSP_NATIVE_G3_HWPROF
 if(msg->type!=DRAW_CMD_G3_CMD_LIST)hwImmUs+=dt;
#endif
}g3ProfCalls++;}
static void SIM_HandleG3CommandImpl(draw_msg_t *msg) {
  u32 temp;
...
#endif
    const void*listPtr=msg->data.ptr;unsigned listSize=msg->size;G3CAT_BEGIN();G3OP_SAMPLE();
#ifdef PSP_NATIVE_LIST_TRACE
      ...
      while (curOffset < msg->size) {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          if(!((kPureOps>>(tempMsg.type&63))&1)||tempMsg.type>=64)pure=false;
           ...
           default:break;}
          // These command payloads already match the native message union.
          // Avoid widening to u64 and copying the entire matrix-sized message.
          switch(tempMsg.type){
          case DRAW_CMD_G3_NORMAL:case DRAW_CMD_G3_COLOR:
          case DRAW_CMD_G3_TEXCOORD:case DRAW_CMD_G3_VTX10:
          case DRAW_CMD_G3_VTXDIFF:case DRAW_CMD_G3_POLYGONATTR:
          case DRAW_CMD_G3_TEXIMAGEPARAM:case DRAW_CMD_G3_TEXPLTTBASE:
          case DRAW_CMD_G3_LIGHTVECTOR:case DRAW_CMD_G3_LIGHTCOLOR:
          case DRAW_CMD_G3_DIFF_AMB:case DRAW_CMD_G3_SPEC_EMI:
            tempMsg.data.numU32=*(u32*)paramPtr;paramPtr+=4;curOffset+=4;
            G3OP_RUN(SIM_HandleG3CommandImpl(&tempMsg));g3ProfCalls++;continue;
          case DRAW_CMD_G3_VTX16:case DRAW_CMD_G3_VTX:
            tempMsg.data.xyz.x=((s16*)paramPtr)[0];tempMsg.data.xyz.y=((s16*)paramPtr)[1];tempMsg.data.xyz.z=((s16*)paramPtr)[2];paramPtr+=8;curOffset+=8;
            G3OP_RUN(SIM_HandleG3CommandImpl(&tempMsg));g3ProfCalls++;continue;
          case DRAW_CMD_G3_VTXXY:
            tempMsg.data.xyz.x=((s16*)paramPtr)[0];tempMsg.data.xyz.y=((s16*)paramPtr)[1];tempMsg.data.xyz.z=prevXYZ.z;tempMsg.type=DRAW_CMD_G3_VTX16;paramPtr+=4;curOffset+=4;
            G3OP_RUN(SIM_HandleG3CommandImpl(&tempMsg));g3ProfCalls+=2;continue;
          case DRAW_CMD_G3_VTXXZ:
            tempMsg.data.xyz.x=((s16*)paramPtr)[0];tempMsg.data.xyz.z=((s16*)paramPtr)[1];tempMsg.data.xyz.y=prevXYZ.y;tempMsg.type=DRAW_CMD_G3_VTX16;paramPtr+=4;curOffset+=4;
            G3OP_RUN(SIM_HandleG3CommandImpl(&tempMsg));g3ProfCalls+=2;continue;
          case DRAW_CMD_G3_VTXYZ:
            tempMsg.data.xyz.y=((s16*)paramPtr)[0];tempMsg.data.xyz.z=((s16*)paramPtr)[1];tempMsg.data.xyz.x=prevXYZ.x;tempMsg.type=DRAW_CMD_G3_VTX16;paramPtr+=4;curOffset+=4;
            G3OP_RUN(SIM_HandleG3CommandImpl(&tempMsg));g3ProfCalls+=2;continue;
...
#elif defined(PSP_NATIVE_G3_HWPROF)
          G3OP_RUN(SIM_HandleG3Command(&tempMsg));
#else
