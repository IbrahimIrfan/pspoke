...
void PSPNativeSoundStep(unsigned bits,unsigned ax,unsigned ay){
...
void PSPNativeSoundPoll(void){SceCtrlData pad;if(sceCtrlPeekBufferPositive(&pad,1)<0){printf("[INPUT] PSP pad read failed\n");abort();}PSPNativeSoundStep(pad.Buttons,pad.Lx,pad.Ly);}
void TP_Init(void){sceCtrlSetSamplingCycle(0);sceCtrlSetSamplingMode(PSP_CTRL_MODE_ANALOG);touchMode=0;previous=0;sampling=FALSE;point=(TPData){128,96,0,TP_VALIDITY_VALID};PSPNativeSoundStep(0,128,128);}
...
int PSPNativeSoundQuitRequested(void){unsigned combo=PSP_CTRL_LTRIGGER|PSP_CTRL_RTRIGGER|PSP_CTRL_SELECT;return (previous&combo)==combo;}

void PSPNativeInputGetRenderState(unsigned*keys,int*mode,int*down,int*x,int*y){
