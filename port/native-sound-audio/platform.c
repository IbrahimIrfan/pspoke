...
void sub_0209A74C(int heap){printf("[SAVE] unsupported backup error UI heap=%d\n",heap);abort();}
void sub_0209AA74(int heap,int error){printf("[SAVE] unsupported backup error UI heap=%d error=%d\n",heap,error);abort();}
static unsigned lockIDs;
s32 OS_GetLockID(void){for(unsigned i=0;i<32;i++)if(!(lockIDs&(1u<<i))){lockIDs|=1u<<i;return 0x40+i;}return OS_LOCK_ID_ERROR;}
void OS_ReleaseLockID(u16 id){if(id<0x40||id>=0x60||!(lockIDs&(1u<<(id-0x40))))abort();lockIDs&=~(1u<<(id-0x40));}
