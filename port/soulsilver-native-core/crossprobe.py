...
cmd=['@PSPDEV@/bin/psp-gcc','-O2','-include',str(shim/'game-pack4.h'),'-Wno-incompatible-pointer-types','-DNONMATCHING','-Wno-implicit-function-declaration','-Wno-int-conversion','-G0','-std=gnu99','-include',str(shim/'compat.h'),'-DPM_KEEP_ASSERTS','-DPSP_NATIVE_OFFLINE','-DSOULSILVER','-DENGLISH','-DSDK_PORT','-DSDK_X86','-DSDK_TS','-DSDK_4M','-DSDK_FINALROM','-DNNS_FINALROM','-DSDK_BUILD_PSP','-ffunction-sections','-fdata-sections']+['-I'+str(p) for p in inc]
def run(p):
