...
int SSFade_CopyWindowRows(const ScreenFade *fade,unsigned char rows[192])
{
    if(fade->type!=5 && fade->type!=8 && fade->type!=9)return -1;
    if(!fade->data || (fade->state!=FADE_ACTIVE && fade->state!=FADE_CLEANUP))return -1;
    const UnkStruct_020120D4 *work=fade->data;
    memcpy(rows,work->unk_00.unk_00[0].unk_C0,192);
    return 1;
}
