...
tool=Path('@PSPDEV@/bin')
flags=['-DPM_KEEP_ASSERTS','-O2','-G0','-std=gnu99','-ffunction-sections','-fdata-sections','-DPOKEPLATINUM_GENERATED_ENUM','-DSDK_VERSION_MAJOR=4','-DSDK_PORT','-DSDK_X86','-DSDK_TS','-DSDK_4M','-DSDK_FINALROM','-DNNS_FINALROM','-I'+str(b/'generated'),'-I'+str(b/'libntrsystem/include'),'-I'+str(b.parent/'native-graphics/libntr/include'),'-I'+str(r/'include'),'-include',str(r/'include/pch/global_pch.h')]
for module in ['math_util','sys_task_manager']:
 with (p/(module+'-build.log')).open('w') as log:subprocess.run([str(tool/'psp-gcc')]+flags+['-c',str(r/'src'/(module+'.c')),'-o',str(p/(module+'.o'))],stdout=log,stderr=log,check=True)
script=subprocess.check_output([str(tool/'psp-gcc'),'-E','-P','-x','assembler-with-cpp','-D__ASSEMBLER__','-I'+str(r/'asm'),'-I'+str(r/'include'),'-I'+str(b/'generated'),str(r/'src/battle/trainer_ai/script.s')],text=True)
(p/'trainer_ai.s').write_text(script+'\n.global gTrainerAITableEnd\ngTrainerAITableEnd:\n')
subprocess.run([str(tool/'psp-as'),'-EL',str(p/'trainer_ai.s'),'-o',str(p/'trainer_ai.o')],check=True)
...
env=dict(os.environ,PSPDEV=str(tool.parent),PATH=str(tool)+os.pathsep+os.environ['PATH'])
