/* PSP_NATIVE_GAME_PROF: coarse game-thread CPU buckets, one card-log line per 600 frames.
   Link-time wraps (Makefile GAME_PROF=1) around a handful of per-frame subsystem entry points.
   Each wrapped call costs two sceKernelGetSystemTimeLow syscalls; ~15 calls per field frame.
...
#define BEGIN unsigned t0=sceKernelGetSystemTimeLow(),g0=g3ProfUs
#define END(id) {unsigned t1=sceKernelGetSystemTimeLow();b[id].us+=t1-t0;b[id].g3+=g3ProfUs-g0;b[id].calls++;}
...
#define ATIME(expr) if(((mallocN+freeN)&63)==0){unsigned t=sceKernelGetSystemTimeLow();expr;allocTimedUs+=sceKernelGetSystemTimeLow()-t;allocTimedN++;}else{expr;}
...
void PSPNativeGameProfLine(unsigned frames){
 if(!frames)return;
 ...
 PSPNativeMemLog("[GPROF-ALLOC] frames=%u malloc=%u free=%u memalign=%u realloc=%u calloc=%u est_us=%llu timed_n=%u (buckets: self_us/g3_us/calls per frame)",frames,mallocN/frames,freeN/frames,memalignN/frames,reallocN/frames,callocN/frames,allocTimedN?(allocTimedUs*(unsigned long long)ops/allocTimedN)/frames:0ULL,allocTimedN);
