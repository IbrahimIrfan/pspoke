...
void NNSi_G3dFuncSbc_BB (NNSG3dRS * rs, u32 opt)
{
    ...
    if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD) &&
        ...
        if (NNS_G3dGlb.flag & NNS_G3D_GLB_FLAG_FLUSH_WVP) {
            ...
            /* PSP opt (opt-tex): borrowed stack bytes, never retained or freed by the renderer */
            (void)msg;
            {
                extern void PSPNativeG3CommandsBorrowed(const void *, unsigned);
                u32 stackBuf[(sizeof(MtxFx43) + sizeof(VecFx32) + sizeof(u32)) / 4];
                stackBuf[0] = GX_PACK_OP(G3OP_MTX_POP, G3OP_MTX_MODE, G3OP_MTX_LOAD_4x3, G3OP_NOP);
                memcpy(stackBuf + 1, &bbcmd1[3], sizeof(MtxFx43) + sizeof(VecFx32));
                PSPNativeG3CommandsBorrowed(stackBuf, sizeof stackBuf);
            }
#else
            ...
            *(u32*)cmdBuf = GX_PACK_OP(G3OP_MTX_POP, G3OP_MTX_MODE, G3OP_MTX_LOAD_4x3, G3OP_NOP);
            ...
            (void)msg;
            {
                extern void PSPNativeG3CommandsBorrowed(const void *, unsigned);
                u32 stackBuf[18];
                memcpy(stackBuf, bbcmd1, 18 * sizeof(u32));
                PSPNativeG3CommandsBorrowed(stackBuf, sizeof stackBuf);
            }
#else
