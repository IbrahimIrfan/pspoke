...
for src in (b/'internalobjects').glob('*.o'):
 ...
 sec=subprocess.check_output(['@PSPDEV@/bin/psp-objdump','-h',str(dst)],text=True);ren=[]
 ...
 if ren:subprocess.run(['@PSPDEV@/bin/psp-objcopy',*ren,str(dst)],check=True)
a=p/'libinternal-overlays.a';a.unlink(missing_ok=True);subprocess.run(['@PSPDEV@/bin/psp-ar','rcs',str(a),*[str(x) for x in out.glob('*.o')]],check=True)
nm=subprocess.check_output(['@PSPDEV@/bin/psp-nm','--defined-only',str(p/'libplatinum-overlays.a')],text=True)
