...
static unsigned dmaCalls;
static void Done(void*){if(s_g3CurColor[0]!=255||s_g3CurColor[1]!=0||s_g3CurColor[2]!=0)abort();dmaCalls++;}
static void scene() {
...
int main(){scePowerSetClockFrequency(333,333,166);scene();s_reg_GX_POWCNT=0x820f;
 ...
 PSPNativeRenderInit();unsigned failures=0,skipped=0,active=0;for(unsigned f=0;f<96;f++){
  ...
  unsigned start=sceKernelGetSystemTimeLow();PSPNativeRenderBegin();u32 commands[2]={0x20,0x001f};MI_SendGXCommandAsyncFast(0,commands,8,Done,NULL);MI_SendGXCommandAsync(0,NULL,0,Done,NULL);triangle();if(PSPNativeRenderPresentNoWait())abort();
  unsigned mask=PSPNativeRenderLastDrawMask();if(mask==1)skipped++;else active++;
  if((f==45||f==46||f==55||f==56||f>=85)&&mask!=3)failures++;
 ...
 if(dmaCalls!=192)failures++;printf("[GX-DMA] callbacks=%u expected=192\n",dmaCalls);
