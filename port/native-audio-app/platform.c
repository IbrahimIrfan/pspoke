...
extern void PSPNativeFatal(const char*);
...
#ifdef PSP_NATIVE_DETERMINISTIC_TICK
/* TEST ONLY: make the game's clock a pure function of the frame count so two builds of
   different speed take identical game paths (A/B exactness runs in PPSSPP). Never release. */
extern unsigned PSPNativeRenderFrameCount(void);
static uint64_t PSPNativeTestClockUs(void){return (uint64_t)PSPNativeRenderFrameCount()*33333ULL;}
#define PSP_NATIVE_CLOCK_US() PSPNativeTestClockUs()
void OS_GetLowEntropyData(u32*data){for(int i=0;i<8;i++)data[i]=0x12345678u^(i*0x9e3779b9u);}
#else
#define PSP_NATIVE_CLOCK_US() sceKernelGetSystemTimeWide()
...
void OS_InitTick(void){tickStartUs=PSP_NATIVE_CLOCK_US();tickBase=0;tickReady=TRUE;}
BOOL OS_IsTickAvailable(void){return tickReady;}
OSTick OS_GetTick(void){
 uint64_t elapsed=PSP_NATIVE_CLOCK_US()-tickStartUs;
...
void OS_SetTick(OSTick value){tickStartUs=PSP_NATIVE_CLOCK_US();tickBase=value;tickReady=TRUE;}
...
void SIM_handleAssertionFailureMsg(const char*file,unsigned line,const char*fmt,...){char m[160];va_list a;va_start(a,fmt);vsnprintf(m,sizeof(m),fmt,a);va_end(a);char b[256];snprintf(b,sizeof(b),"SDK assertion %s:%u %s",file,line,m);PSPNativeFatal(b);}
...
void OS_WaitIrq(BOOL clear,OSIrqMask bits){
 if(bits!=OS_IE_V_BLANK){char b[64];snprintf(b,sizeof(b),"unsupported IRQ wait %08lx",(unsigned long)bits);PSPNativeFatal(b);}
...
void SIM_handleAssertionFailure(const char*file,unsigned line,const char*expr){char b[256];snprintf(b,sizeof(b),"assertion %s:%u %s",file,line,expr);PSPNativeFatal(b);}
void OS_IrqHandler(void){PSPNativeFatal("unsupported direct DS network IRQ handler");}
void OS_ResetSystem(u32 parameter){char b[96];snprintf(b,sizeof(b),"reset requested %lu; native restart unsupported",(unsigned long)parameter);PSPNativeFatal(b);}
...
/* The renderer computes the clip matrix lazily; the only SDK reader of the CLIPMTX_RESULT
   registers is G3X_GetClipMtx, wrapped here so a read always sees the current matrix. */
