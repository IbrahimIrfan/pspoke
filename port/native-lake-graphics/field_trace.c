...
static BOOL HandleInputsEventsAndProcesses(FieldSystem *fieldSystem)
{
    unsigned frame=PSPNativeRenderFrameCount();{static int previous=-1;Party*party=SaveData_GetParty(fieldSystem->saveData);int count=Party_GetCurrentCount(party);if(count!=previous){previous=count;printf("[PARTY] frame=%u count=%d",frame,count);if(count){Pokemon*mon=Party_GetPokemonBySlotIndex(party,0);printf(" species=%u level=%u exp=%u",Pokemon_GetValue(mon,MON_DATA_SPECIES,NULL),Pokemon_GetValue(mon,MON_DATA_LEVEL,NULL),Pokemon_GetValue(mon,MON_DATA_EXPERIENCE,NULL));}puts("");}}if(frame%30==0&&fieldSystem->location&&fieldSystem->playerAvatar)printf("[FIELD] frame=%u map=%d x=%d z=%d\n",frame,fieldSystem->location->mapHeaderID,PlayerAvatar_GetXPos(fieldSystem->playerAvatar),PlayerAvatar_GetZPos(fieldSystem->playerAvatar));
