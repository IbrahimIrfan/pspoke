...
{
    printf("[MENU-TRACE] before Heap_Create(HEAP_ID_APPLICATION, HEAP_ID_MAIN_MENU, HEAP_SIZE_MAIN_MENU);\n");
    ...
    printf("[MENU-TRACE] before MainMenuUtil_Init(HEAP_ID_MAIN_MENU);\n");
    ...
    printf("[MENU-TRACE] before Sound_ConfigureBGMChannelsAndReverb(SOUND_CHANNEL_CONFIG_DEFAULT);\n");
...
{ printf("[MENU-TRACE] state=%d\n",*state);
    MainMenuAppData *appData = ApplicationManager_Data(appMan);

    appData->framesCounter++;
    printf("[MENU-TRACE] before CTRDG_IsExisting();\n");
    CTRDG_IsExisting();
    ...
    printf("[MENU-TRACE] before InitMainMenuGraphics(appData);\n");
        InitMainMenuGraphics(appData);
        *state = MAIN_MENU_STATE_CHECK_WFC_USER_INFO;
        break;
    case MAIN_MENU_STATE_CHECK_WFC_USER_INFO:
    printf("[MENU-TRACE] before if (CheckWFCUserInfoErased(appData) == FALSE) {\n");
